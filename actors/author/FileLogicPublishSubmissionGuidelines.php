...
$sql = "SELECT id,name,size,downloads FROM submissionguidelines";
...
//Added others
    ...
    if (!in_array($extension, ['pdf'])) {
        ...
        if(move_uploaded_file($file, $destination)) {
            ...
            $sql = "INSERT INTO submissionguidelines(name, size, downloads) VALUES ('$filename', $size, 0)";
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT id,name,size,downloads FROM submissionguidelines WHERE id=$id";
    $result = mysqli_query($conn, $sql);
    ...
    if (file_exists($filepath)) {
        ...
        header('Content-Length: ' . filesize('../../media/' . $file['name']));
        readfile('../../media/' . $file['name']);

        // Now update downloads count
        $newCount = $file['downloads'] + 1;
        $updateQuery = "UPDATE submissionguidelines SET downloads=$newCount WHERE id=$id";

   ...
   <div id="main-wrapper"> 
     ...
     <?php
         if(isset($_POST["changePass"])){
           $aEmail = $_SESSION['p_email'];
           $currentPass = $_POST['currentPass'];
           $newPass = $_POST['newPwd']; 
           $conNewPass = $_POST['CNPwd'];
           
           $query_result = mysqli_query($con,"select password from publicationchair where emailpubchair='$aEmail'");
           ...
           if($row['password'] == $encriptedCurrentPass){
              ...
              if($newPass == $conNewPass){
                ...
                if($currentPass <> $newPass){  
                  ...
                  and (user_type <> 'PublicationChair')");
                  $CDCount = 0;
                  ...
                  if($CDCount == 0){
                    ...
                    $query1_r = mysqli_query($con,"update userinfotable set password='$encriptedNewPass'
                                where (email = '$aEmail') and (user_type = 'PublicationChair')");
                    
                    $query2_r = mysqli_query($con,"update publicationchair set password='$encriptedNewPass' where emailpubchair = '$aEmail'");
